// Compile the Lennard-Jones version:
// gcc -DUSE_LJ simulating_crystal.c energy.c lennard_jones.c -o simulating_lj.exe -std=c11 -O2 -Wall -Wextra -lm

// Compile the Stillinger-Weber version:
// gcc -DUSE_SW simulating_crystal.c energy.c stillinger_weber.c -o simulating_sw.exe -std=c11 -O2 -Wall -Wextra -lm
...
#if defined(USE_LJ) && defined(USE_SW)
#error "Choose only one potential: USE_LJ or USE_SW."
#endif

#if !defined(USE_LJ) && !defined(USE_SW)
#error "No potential selected. Compile with -DUSE_LJ or -DUSE_SW."
#endif
...
static int run_selected_energy_calculation(
    int structure_choice,
    const char *element,
    double lattice_constant,
    int nx,
...
)
{
    ...
    // Number of lattice points in the finite block
    *number_of_atoms =
        (nx + 1) * (ny + 1) * (nz + 1);
...
)
{
    ...
    // Number of atoms in the finite block
    *number_of_atoms =
          (nx + 1) * (ny + 1) * (nz + 1)
        + nx * ny * nz;
...
{
    vectors[0][0] = lattice_constant;
    vectors[0][1] = 0.0;
    vectors[0][2] = 0.0;

    vectors[1][0] = 0.0;
    vectors[1][1] = lattice_constant;
    vectors[1][2] = 0.0;

    vectors[2][0] = 0.0;
    vectors[2][1] = 0.0;
    vectors[2][2] = lattice_constant;
}
...
// Calculate the energy for the selected potential
...
)
{
    ...
    if (number_of_energy_atoms == 0) {
        printf(
            "Unable to determine the number of "
            "periodic energy atoms.\n"
        );
    ...
    Atom *energy_atoms =
        malloc(
            number_of_energy_atoms
            * sizeof *energy_atoms
        );

    if (energy_atoms == NULL) {
        printf(
    ...
    size_t generated_count =
        generate_periodic_cubic_supercell(
            energy_atoms,
            number_of_energy_atoms,
            structure_choice,
            element,
    ...
    if (generated_count != number_of_energy_atoms) {
        printf(
            "Unable to generate the periodic "
            "energy supercell.\n"
        );
    ...
    double lx = nx * lattice_constant;
    double ly = ny * lattice_constant;
    double lz = nz * lattice_constant;

    printf("\nPeriodic energy supercell\n");

    printf(
        "Number of unique periodic atoms: %zu\n",
        number_of_energy_atoms
    );

    printf(
        "Supercell dimensions: "
        "%.8f x %.8f x %.8f Angstrom\n",
        lx,
        ly,
        lz
    );
    ...
    printf("\nLennard-Jones potential selected\n");

    printf("Enter epsilon in eV: ");
    ...
    printf("Enter sigma in Angstrom: ");
    ...
    printf("Enter the LJ cutoff in Angstrom: ");

    if (scanf("%lf", &cutoff) != 1 ||
        cutoff <= 0.0) {
    ...
    if (!minimum_image_cutoff_is_valid(
            lx,
            ly,
            lz,
            cutoff)) {
        ...
        printf(
            "The cutoff must be less than half "
            "of every supercell length.\n"
        );

        printf(
            "Increase the periodicity or use a "
            "smaller cutoff.\n"
        );
    ...
    if (!isfinite(total_energy)) {
        printf(
            "LJ energy calculation failed.\n"
        );

        printf(
            "Check for duplicated atoms and "
            "zero PBC distances.\n"
        );
    ...
    printf(
        "Total LJ energy: %.12f eV\n",
        total_energy
    );

    printf(
        "LJ energy per atom: %.12f eV/atom\n",
        total_energy
        / (double)number_of_energy_atoms
    );
    ...
    if (structure_choice != 4) {
        printf(
            "\nThe supplied SW parameters are intended "
    ...
    double cutoff =
        SILICON_SW_PARAMETERS.cutoff_a
        * SILICON_SW_PARAMETERS.sigma;

    printf("\nStillinger-Weber potential selected\n");

    printf(
        "SW cutoff: %.10f Angstrom\n",
        cutoff
    );
    ...
    if (!minimum_image_cutoff_is_valid(
        ...
        printf(
            "The supercell is too small "
            "for the SW cutoff.\n"
        );

        printf(
            "Use at least a 2 x 2 x 2 "
            "diamond conventional supercell.\n"
        );
    ...
    double total_energy =
        calculate_sw_total_energy(
            energy_atoms,
            number_of_energy_atoms,
            lx,
            ly,
            lz,
            &SILICON_SW_PARAMETERS,
            &two_body_energy,
            &three_body_energy
        );
    ...
    printf("\nStillinger-Weber results\n");

    printf(
        "SW two-body energy: %.12f eV\n",
        two_body_energy
    );

    printf(
        "SW three-body energy: %.12f eV\n",
        three_body_energy
    );

    printf(
        "SW total energy: %.12f eV\n",
        total_energy
    );

    printf(
        "SW energy per atom: %.12f eV/atom\n",
        total_energy
        / (double)number_of_energy_atoms
    );
...
int main(void)
{
    ...
    printf(
        "Number of visualization atoms: %d\n",
        number_of_atoms
    );

    // Calculate the selected potential energy
    if (!run_selected_energy_calculation(
            structure_choice,
            element,
            lattice_constant,
            nx,
        ...
        printf(
            "The energy calculation was not completed.\n"
        );
